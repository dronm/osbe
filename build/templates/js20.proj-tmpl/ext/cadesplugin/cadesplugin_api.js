var cadesPluginAPI = function () {
    ...
    function show_firefox_missing_extension_dialog()
    {
        ...
        {  
            ...
            document.getElementById("cadesplugin_close_install").addEventListener('click',function()
                                    {
                                        plugin_loaded_error("КриптоПро ЭЦП браузер плагин недоступен");
    ...
    function plugin_loaded_error(msg)
    {
        ...
        if(typeof(msg) == 'undefined' || typeof(msg) == 'object')
            msg = "КриптоПро ЭЦП браузер плагин недоступен";
        plugin_resolved = 1;
        if(canPromise)
        {
            plugin_reject(msg);
    ...
    function check_load_timeout()
    {
        ...
        if(canPromise)
        {
            plugin_reject("Истекло время ожидания загрузки плагина КриптоПро ЭЦП");
        } else {
            window.postMessage("cadesplugin_load_error", "*");
        }
    ...
    function check_npapi_plugin (){
        try {
            ...
            if (mimetype) {
                ...
                if (plugin) {
                    plugin_loaded_error("КриптоПро ЭЦП браузер плагин загружен, но не создаются обьекты");
                }else
                {
                    plugin_loaded_error("Ошибка при загрузке КриптоПро ЭЦП браузер плагина");
                }
            }else
            {
                plugin_loaded_error("КриптоПро ЭЦП браузер плагин недоступен");
    ...
    function check_plugin_working()
    {
        ...
        if (isIeLessThan9) {
            plugin_loaded_error("Internet Explorer версии 8 и ниже не поддерживается КриптоПро ЭЦП браузер плагином");
            return;
        }
