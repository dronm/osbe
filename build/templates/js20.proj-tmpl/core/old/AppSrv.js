...
AppSrv.prototype.send = function(func, argv, queryId){
...
AppSrv.prototype.onMessage = function(message){
	...
	var resp; //response object ResponseJSON/ResponseXML
	...
	var ev_model_id = window.getApp().EVENT_MODEL_ID;
	...
	if(window["ResponseJSON"]){
		...
		//version comparing
		var br_vers = window.getApp().getServVar("version");
		//only if browser verion is lower
		vers_error = (!error_n
				&& m.app_version
				&& m.app_version!=br_vers
				&& parseInt(m.app_version.replaceAll(".", ""),10) > parseInt(br_vers.replaceAll(".", ""),10)
			);
 ...
 */
AppSrv.prototype.runPublicMethod = function(func,argv,progressCallBack,callBack){	
