...
function ViewGridEditInlineAjx(id,options){
	...
	options.model = (options.models && options.models[gridModelId])? options.models[gridModelId] : new window[gridModelId]();
...
//debugger
//console.log("ViewGridEditInlineAjx.prototype.setWritePublicMethod")
		...
		for (var col_id=0;col_id<columns.length;col_id++){
			...
			if (column.getField()){
				...
				if (pm.fieldExists(f_id)){
					...
					console.log("ViewGridEditInlineAjx.prototype.setWritePublicMethod: fieldID: "+f_id+", no PublicMethod field")
				}
			}
		}
	...
	for (let colId=0; colId<columns.length; colId++){
		...
		if (column.getField()){
			bindings.push(new DataBinding({
				"field":model.getField(column.getField().getId()),
				"model":model,
				"control":this.getElement(column.getId())
			}));
