...
function BankEditRef(id,options){
	...
	options.placeholder= "Введите БИК банка для поиска";
	...
	options.selectFormatFunction = options.selectFormatFunction || this.m_descrFunction;
	...
	//options.acDescrFields = [options.acModel.getField("bik")];
	
	options.acDescrFunction = options.acDescrFunction || this.m_descrFunction;
	...
	/*options.onSelect = options.onSelect || function(f){
		if (self.m_view){
			self.m_view.m_bankDescr = self.m_descrFunction(f);//fields["name"].getValue();
		}		
	}*/
