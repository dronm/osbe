...
WindowMessage.prototype.toDOM = function(){
	...
	var mw_st = WIN_MESSAGE_STYLE; //window.getApp().getWinMessageStyle();
	...
	if (!this.m_window){
		var n = CommonHelper.getElementsByAttr("windowMessage", window.body, "class", true);
		if (!n.length){
	...
	for (var i=this.m_messages.length-1;i>=0;i--){
		...
		cont.addElement(new Control(this.m_messages[i].id+":head","DIV",
			{"value":CommonHelper.dateFormat(this.m_messages[i].date_time,"H:i:s")
			}));				
		//console.log("content=")
